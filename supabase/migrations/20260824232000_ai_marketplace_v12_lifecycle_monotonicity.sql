...
begin
  ...
  -- Unpaid Checkout Sessions do not yet have an order. Mark their durable
  -- checkout attempt directly so expired/failed sessions cannot remain stale.
  ...
  -- A checkout failure/expiration arriving after a paid order must never
  -- downgrade that paid order. The attempt event remains recorded above.
  ...
  -- Terminal/restrictive states are monotonic. Recovery may only restore a
  -- failed renewal, or a won dispute represented by charge.dispute.closed.
