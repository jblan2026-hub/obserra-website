-- Reactivate Academy access only after a distinct, verified replacement payment.
-- Source only until the controlled Academy migration window is authorized.
-- This migration creates no payment, learner, refund, dispute, or FDACS record.
...
begin
  ...
  if exists (
    ...
    where r.checkout_session_id = p_checkout_session_id
      and r.payment_intent_id = nullif(p_payment_intent_id, '')
      and r.course_slug = p_course_slug
      and r.course_version = p_course_version
  ...
  insert into public.academy_learner_state (
    ...
    set access_status = 'active',
        ...
        record_version = public.academy_learner_state.record_version + 1,
        updated_at = now()
    where public.academy_learner_state.access_status in ('refunded', 'revoked')
      and public.academy_learner_state.payment_reference is distinct from excluded.payment_reference
      and not exists (
        select 1 from public.academy_payment_reversal_events r
        where r.checkout_session_id = excluded.payment_reference
          and r.course_slug = excluded.course_slug
          and r.target_access_status in ('refunded', 'revoked')
      )
  ...
  if not found then
    select * into v_state from public.academy_learner_state
      where clerk_user_id = v_event.clerk_user_id and course_slug = p_course_slug
      for update;
  end if;
  if not found or
     v_state.access_status <> 'active' or
     v_state.payment_reference is distinct from p_checkout_session_id or
     v_state.course_version is distinct from p_course_version then
    raise exception 'Verified payment did not activate exact Academy access';
  end if;
...
begin
  ...
  if exists (
    select 1 from public.academy_payment_reversal_events r
    where r.checkout_session_id = p_checkout_session_id
      and r.payment_intent_id = v_event.payment_intent_id
      and r.course_slug = p_course_slug
      and r.course_version = v_event.course_version
      and r.target_access_status in ('refunded', 'revoked')
  ) then
    raise exception 'Reversed payment cannot activate Academy access';
  end if;
  ...
  if not found or
     v_state.access_status <> 'active' or
     v_state.payment_reference is distinct from p_checkout_session_id or
     v_state.course_version is distinct from v_event.course_version then
    raise exception 'Verified payment did not activate exact Academy access';
  end if;
...
revoke all on function public.academy_record_paid_checkout(text, text, text, text, text, text, text, text, text) from public, anon, authenticated, service_role;
revoke all on function public.academy_claim_paid_checkout(text, text, text, text, text) from public, anon, authenticated, service_role;
