...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!entry) return { title: "Application not found", robots: { index: false, follow: false } };
  ...
  return {
    ...
    openGraph: {
      ...
      type: "website",
      images: [{ url: "/brand/visuals/obserra-eios-intelligence-hero.png", width: 1672, height: 941, alt: `${entry.name} enterprise application` }],
    },
    twitter: {
...
export default async function AppDetailPage({ params }: Props) {
  ...
  const canonicalUrl = `https://www.obserrallc.com/apps/${entry.slug}`;
    ...
    "@graph": [
      {
        ...
        applicationSuite: "Obserra Applications",
        description: entry.value,
        operatingSystem: "Web",
        url: canonicalUrl,
        ...(liveApplicationUrl ? { installUrl: liveApplicationUrl } : {}),
        featureList: entry.features,
        softwareRequirements: entry.deployment.join(", "),
        publisher: {
          "@type": "Organization",
  ...
  return (
    <main className="app-detail-page apps-page">
      ...
      <section className="app-detail-hero">
        <span className={`status-pill ${entry.status === "Available" ? "status-available" : entry.status === "Pilot" ? "status-pilot" : "status-coming"}`}>{entry.status}</span>
        <h1>{entry.name}</h1>
        <p>{entry.value}</p>
      ...
      <section className="app-pricing">
        ...
        <p>{entry.pricing}</p>
        <p>
          Purchase through the website, manage billing in Stripe, launch SaaS applications, or download approved deployment packages.
          Access is revalidated against subscription status and automatically denied when a subscription is unpaid, canceled, incomplete, or expired.
        </p>
        <div className="apps-actions">
          {liveApplicationUrl ? (
            <a className="apps-button" href={liveApplicationUrl} target="_blank" rel="noopener noreferrer">Subscribe &amp; Launch</a>
          ) : entry.status !== "Coming Soon" ? (
            <Link className="apps-button" href={`/apps/${entry.slug}/subscribe`}>Choose subscription</Link>
          ) : (
            <Link className="apps-button" href={`/contact?interest=application-preview&app=${entry.slug}`}>Request preview</Link>
          )}
          {liveApplicationUrl ? (
            <a className="apps-outline" href={liveApplicationUrl} target="_blank" rel="noopener noreferrer">Open live application</a>
          ) : (
            <a className="apps-outline" href={`/api/apps/access?app=${entry.slug}`}>Launch SaaS</a>
          )}
