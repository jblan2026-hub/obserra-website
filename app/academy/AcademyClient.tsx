...
export default function AcademyClient() {
  ...
  return <main>
    ...
    <section className="hero">
      <p className="kicker">Obserra Academy · Obserra Technologies</p>
    ...
    <section className="catalog" id="courses">
      ...
      <div className="course-grid">{visibleCourses.map((course) => <button key={course.id} className={selected.id === course.id ? "course-card selected" : "course-card"} onClick={() => chooseCourse(course.id)}>
        <span>{course.department} · {course.track}</span><h3>{course.title}</h3><p>{course.description}</p><footer><b>{money.format(course.price)}</b><em>{course.duration}</em></footer>
      </button>)}</div>
    </section>
