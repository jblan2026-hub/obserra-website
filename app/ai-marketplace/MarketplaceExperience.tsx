...
function CapabilityUniverse({ nodes, selected, onSelect }: { nodes: SceneNode[]; selected: string | null; onSelect: (node: SceneNode) => void }) {
  const [renderer, setRenderer] = useState<"loading" | "ready" | "recovering" | "unavailable">("loading"), [reducedMotion, setReducedMotion] = useState(false), [cluster, setCluster] = useState(""), [sceneFamily, setSceneFamily] = useState("");
  ...
  const clusters = useMemo(() => [...new Set(nodes.map((node) => node.scene_cluster))].sort((left, right) => left.localeCompare(right)), [nodes]), families = useMemo(() => [...new Set(nodes.map((node) => node.family))].sort((left, right) => left.localeCompare(right)), [nodes]), visibleNodes = useMemo(() => nodes.filter((node) => (!cluster || node.scene_cluster === cluster) && (!sceneFamily || node.family === sceneFamily)), [cluster, nodes, sceneFamily]), selectedNode = nodes.find((node) => node.product_id === selected) ?? null;
  ...
  return <div className="ai-marketplace__universe" data-webgl={renderer} role="application" tabIndex={0} onKeyDown={keyboardNode} aria-label="Interactive 3D catalog capability universe. Arrow keys select nodes; Enter opens the selected catalog record.">
    ...
    <div className="ai-marketplace__scene-controls" aria-label="3D capability filters"><label htmlFor="marketplace-scene-cluster">Cluster <select id="marketplace-scene-cluster" value={cluster} onChange={(event) => setCluster(event.target.value)}><option value="">All clusters</option>{clusters.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label htmlFor="marketplace-scene-family">Family <select id="marketplace-scene-family" value={sceneFamily} onChange={(event) => setSceneFamily(event.target.value)}><option value="">All families</option>{families.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><button type="button" onClick={() => { setCluster(""); setSceneFamily(""); }}>Reset</button><span>{visibleNodes.length} object{visibleNodes.length === 1 ? "" : "s"}</span></div>
    <aside className="ai-marketplace__scene-selection" aria-live="polite">{selectedNode ? <><span>Selected capability</span><strong>{selectedNode.name}</strong><small>{selectedNode.family} · {selectedNode.proficiency ?? selectedNode.product_type} · {selectedNode.relationship_product_ids.length} declared relationship{selectedNode.relationship_product_ids.length === 1 ? "" : "s"}</small><Link href={`/ai-marketplace/${encodeURIComponent(selectedNode.slug)}`}>Open capability record <span aria-hidden="true">→</span></Link></> : <><span>Capability selection</span><strong>Choose a labelled 3D object.</strong><small>Use Family and Cluster to narrow the real objects in this bounded catalog scene.</small></>}</aside>
