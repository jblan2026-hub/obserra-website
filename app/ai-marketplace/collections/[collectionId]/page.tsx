...
export default async function MarketplaceCollectionPage({ params, searchParams }: Props) {
  ...
  const detail = marketplaceV12PedestalDetail(collection), publicDetail = marketplacePublicProductDetail(detail), commerce = await marketplaceV12ProductCommerce(collection), path = marketplaceV12PublicPath(collection);
