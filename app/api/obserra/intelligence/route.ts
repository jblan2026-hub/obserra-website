...
export async function GET(request: NextRequest) {
  if (!authorized(request)) {
    return NextResponse.json(
      { schemaVersion: "1.1", sourceId: "website", status: "unauthorized", error: "Valid owner intelligence credentials are required." },
  ...
  const recommendations: Array<{ severity: "low" | "medium" | "high" | "critical"; title: string; recommendation: string }> = [];
  if (!identityReady) recommendations.push({ severity: "critical", title: "Production identity is not fully configured", recommendation: "Configure Clerk production credentials and verify protected learner, portal, and administrative routes before promotion." });
  if (!stripeReady) recommendations.push({ severity: "critical", title: "Stripe payment processing is unavailable", recommendation: "Configure the Stripe secret key and validate checkout session creation in preview before production promotion." });
  if (!webhookReady) recommendations.push({ severity: "critical", title: "Stripe webhook verification is unavailable", recommendation: "Configure the Stripe webhook secret. Checkout remains fail closed until verified fulfillment is available." });
  ...
  const payload = {
    ...
    memory: `Website intelligence observed ${PUBLIC_ROUTES.length} public routes, ${PROTECTED_ROUTES.length} protected route patterns, commerce readiness ${commerceReady ? "operational" : "degraded"}, and governed remediation readiness enabled.`,
    services: {
    ...
    identity: { provider: "clerk", configured: identityReady, status: identityReady ? "operational" : "degraded" },
    commerce: {
      provider: "stripe",
    ...
    remediation: {
      ...
      detectionMappingsRequired: ["MITRE ATT&CK", "OWASP Top 10"],
      ...
      validationCommand: "npm run verify:academy-release",
    },
    deployment: {
      provider: "vercel",
