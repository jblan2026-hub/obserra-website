...
import { marketplaceV12BindingReviewProgress, recordMarketplaceV12BindingImportReview } from "../../../../../lib/marketplace-v12-binding-import";
import { marketplaceV12CommerceSubjects, marketplaceV12Product, marketplaceV12Summary } from "../../../../../lib/marketplace-v12-catalog";
import { marketplaceV12Offer, marketplaceV12PurchaseOptions, type MarketplaceV12PurchaseOption } from "../../../../../lib/marketplace-v12-bindings";
...
function priceMatches(price: Stripe.Price, stripeProductId: string, expected: { kind: string; cadence?: string; amount_minor: number }, expectedKey: string, option: MarketplaceV12PurchaseOption, live: boolean, productId: string, revision: string, artifactSha256: string) {
  const stripeProductReference = typeof price.product === "string" ? price.product : price.product.id;
  const recurring = expected.kind === "recurring";
  return price.active && price.livemode === live && stripeProductReference === stripeProductId && price.currency === "usd" && price.unit_amount === expected.amount_minor
    ...
    && price.metadata.bindingKey === expectedKey
    && price.metadata.purchaseOption === option
    && price.metadata.commerceSource === SOURCE;
}

function requiredOfferBindings() {
  return marketplaceV12CommerceSubjects().reduce((total, subject) => {
    const product = marketplaceV12Product(subject.productId);
    return total + (product ? marketplaceV12PurchaseOptions(product).length : 0);
  }, 0);
}

async function progress(productId: string) {
  const revision = marketplaceV12Summary().revision;
  const evidence = await marketplaceV12BindingReviewProgress(revision, productId);
  return {
    contract: "obserra-marketplace-v12-binding-review-progress-v1",
    ...evidence,
    requiredProductCards: marketplaceV12CommerceSubjects().length,
    requiredOfferBindings: requiredOfferBindings(),
    checkoutActivated: false,
    bindingManifestUpdated: false,
    releaseEvidenceUpdated: false,
  };
}
...
export async function GET(request: Request) {
  ...
  if (!product || !subject || product.product_type === "collection" || product.product_type === "bundle") return publicFailure(409, "catalog_subject_invalid");
  try { return NextResponse.json(await progress(product.product_id), { headers: { "cache-control": "no-store", "x-robots-tag": "noindex, nofollow" } }); }
  catch { return publicFailure(503, "binding_progress_unavailable"); }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (matches.has_more || existing.length > 1) return publicFailure(409, "ambiguous_stripe_product");
    ...
    if (!productMatches(stripeProduct, product.product_id, revision, subject.artifactSha256)) return publicFailure(409, "stripe_product_governance_failed");
    const prices = await stripe.prices.list({ product: stripeProduct.id, active: true, limit: 100 });
    const resolved = prices.data.filter((candidate) => priceMatches(candidate, stripeProduct.id, offer, key, option, live, product.product_id, revision, subject.artifactSha256));
    if (prices.has_more || resolved.length > 1) return publicFailure(409, "ambiguous_stripe_price");
    const priceMetadata = { obserraMarketplaceProduct: product.product_id, catalogRevision: revision, artifactSha256: subject.artifactSha256, bindingKey: key, purchaseOption: option, commerceSource: SOURCE };
    const stripePrice = resolved[0] ?? await stripe.prices.create({ product: stripeProduct.id, currency: "usd", unit_amount: offer.amount_minor, ...(offer.kind === "recurring" ? { recurring: { interval: offer.cadence as "month" | "year", interval_count: 1 } } : {}), metadata: priceMetadata }, { idempotencyKey: `obserra-v12-price-${revision.slice(0, 16)}-${product.product_id}-${key}` });
    if (!priceMatches(stripePrice, stripeProduct.id, offer, key, option, live, product.product_id, revision, subject.artifactSha256)) return publicFailure(409, "stripe_price_governance_failed");
    ...
    return NextResponse.json({ reviewed: true, purchaseOption: option, stripeProductId: stripeProduct.id, stripePriceId: stripePrice.id, ...(await progress(product.product_id)) }, { headers: { "cache-control": "no-store", "x-robots-tag": "noindex, nofollow" } });
  } catch (error) {
