...
export async function POST(request: Request) {
  ...
  let body: { recipient?: unknown; reason?: unknown; idempotencyKey?: unknown; subject?: unknown };
  try {
  ...
  if (!validEmail(recipient) || reason.length < 5 || reason.length > 500 || operationId.length > 200) {
    return response({ error: "invalid-export-request" }, 400);
  }
  ...
  try {
    ...
    const send = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [recipient],
        subject,
        text: "Attached is the Obserra Governance, Security, Privacy, and Release Evidence Package generated from the protected Governance Center.",
        attachments: [{ filename: `obserra-governance-evidence-${new Date().toISOString().slice(0, 10)}.pdf`, content: pdf.toString("base64") }],
      }),
    });
    ...
    return response({ accepted: true, operationId, recipientDomain: recipient.split("@")[1] ?? null }, 202);
  } catch {
    return response({ error: "governance-email-unavailable" }, 503);
  }
}
