...
function errorResponse(error: unknown) {
  if (error instanceof FloridaClassDOwnerPreviewAuthorizationError || error instanceof FloridaClassDOwnerCoursewareError) {
  ...
  console.error(
    "Florida Class D owner-preview courseware action failed",
    error instanceof Error ? error.name : "unknown_error",
  );
  return NextResponse.json(
    { error: "The protected owner courseware action could not be completed.", code: "FDACS_OWNER_COURSEWARE_FAILED" },
    { status: 503, headers },
  );
}
  ...
  try {
    ...
    if (action === "create-upload") {
      if (
        ...
        return NextResponse.json(
          { error: "Complete courseware file metadata is required.", code: "FDACS_OWNER_COURSEWARE_METADATA_REQUIRED" },
          { status: 400, headers },
        );
      }
    ...
    return NextResponse.json(
      { error: "Unsupported owner courseware action.", code: "FDACS_OWNER_COURSEWARE_ACTION_INVALID" },
      { status: 400, headers },
    );
  } catch (error) {
    return errorResponse(error);
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!body || typeof body.objectPath !== "string") {
      return NextResponse.json(
        { error: "A protected courseware object path is required.", code: "FDACS_OWNER_COURSEWARE_PATH_REQUIRED" },
        { status: 400, headers },
      );
    }
