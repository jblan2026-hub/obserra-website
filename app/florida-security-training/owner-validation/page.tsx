  ...
  return (
    <main className="fl-classd">
      <section className="fl-classd__hero">
        ...
        <p className="fl-classd__lead">This production owner surface separates authenticated inspection from governed provider execution. You can inspect readiness after AAL2 authentication even when an exact-release provider action remains blocked.</p>
        ...
        {!configuration.authorized ? (
          <div className="fl-classd__notice is-locked" role="status">
            ...
            <div>
              ...
              <span>Provider actions remain unavailable until every exact-release validation control is satisfied. Blocking controls: {configuration.blockingKeys.join(", ") || "unknown"}.</span>
            </div>
          </div>
        ) : null}
      </section>
      ...
      <section className="fl-classd__section">
        <div className="fl-classd__section-heading">
          ...
          <h2>Inspect the governed LMS lifecycle</h2>
          <p>Provider actions are enabled only where a dedicated exact-release validation path exists. Locked modules remain visible as lifecycle controls rather than linking to an authorization dead end.</p>
        </div>
          ...
          {modules.map(([title, href, Icon, detail], index) => (
            <div key={title}>
              ...
              <span>
                ...
                <br />
                {href ? (
                  <Link href={href}>{configuration.authorized ? "Open governed validation" : "Inspect validation readiness"}</Link>
                ) : (
