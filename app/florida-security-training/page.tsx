  ...
  return (
    <main className="fl-classd">
      <section className="fl-classd__hero">
        ...
        <p className="fl-classd__lead">A production-stack learning environment from <strong>{FLORIDA_CLASS_D_COURSE.provider}</strong>, engineered for live instruction, secure enrollment controls, verified attendance, student interaction, learning checks, examination controls, training records, and post-course administration. The software uses the production architecture, while regulated course credit remains disabled until the separate FDACS activation requirements are satisfied.</p>
        <div className="fl-classd__notice"><LockKeyhole size={20} /><div><strong>Coming Soon.</strong><span>Enrollment, payment, and student course access are not open. FDACS provider and course authorization have not been granted. Production instruction, course credit, completion, certificates, and LIAS reporting are disabled. Production validation cannot create an authorized student completion or licensing record.</span></div></div>
      ...
      <section className="fl-classd__section">
        <div className="fl-classd__section-heading"><span>CONTROLLED CURRICULUM ARCHITECTURE</span><h2>Four planned live lessons every authorized training day</h2><p>The controlled design provides four 120-minute live instructional lessons per day, with a 15-minute break after Lessons 1, 2, and 3. During production software validation, the LMS verifies separate tracking for instruction, breaks, connection time, security-question responses, attendance, and participation without awarding regulated instructional credit. Break time is recorded but is never credited toward the required 40 instructional hours. The certification examination is controlled separately from the 40 instructional hours.</p></div>
      ...
      <section className="fl-classd__section fl-classd__automation">
        <div className="fl-classd__section-heading"><span>REGULATED LMS PRODUCTION VALIDATION</span><h2>Instructor-led and interaction rich</h2><p>The production-stack classroom validates real-time instructor presence and auditable student participation rather than passive video playback. Validation results do not establish FDACS approval or production authorization for regulated training outcomes.</p></div>
      ...
      <section className="fl-classd__section">
        <div className="fl-classd__section-heading">
          ...
          <p>Identity requirements are checked before student sign-in. The student starts with a public no-PII readiness gate, then enters the protected account, enrollment, hosted identity, instructor-attestation, and one-device course-access sequence. Identity images remain with the hosted verification provider and are not copied into the LMS.</p>
        </div>
