...
import RecordedMakeupPlayer from "./RecordedMakeupPlayer";
  ...
  return (
    <main className="fdacs-live fdacs-makeup">
      ...
      <section className="fdacs-live__panel fdacs-makeup__summary">
        ...
        <p>Only school-assigned recovery work appears here. Recorded make-up is limited to the controlled absence-recovery allowance and is never treated as a substitute for the live Class D course. Only server-verified playback time may advance a recorded make-up assignment.</p>
      </section>
      ...
      <section className="fdacs-makeup__assignments">
        {assignments.map((assignment) => (
          <article className="fdacs-live__panel fdacs-makeup__assignment" key={assignment.id}>
            ...
            {assignment.delivery_method === "recorded_makeup" && !["cancelled", "failed", "certified"].includes(assignment.status) ? <RecordedMakeupPlayer assignmentId={assignment.id} assignedMinutes={assignment.assigned_minutes} /> : <div className="fdacs-makeup__notice">The school will coordinate this make-up assignment through the regulated instructor workflow.</div>}
