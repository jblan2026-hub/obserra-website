...
export default function ObserraGuidePanel({ onClose }: Props) {
  ...
  return (
    <section className={styles.panel}>
      <header className={styles.header}>
        ...
        <button type="button" onClick={onClose} aria-label="Minimize Obserrian">−</button>
      </header>
      ...
      <div className={styles.quick} aria-label="Suggested Obserrian actions">
        {context.prompts.map((item) => (
          <button type="button" key={item} onClick={() => appendExchange(item)}>{item}</button>
        ))}
      </div>
