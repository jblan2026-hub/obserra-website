...
export default function EiosShowcase() {
  ...
  return <main className="eios-page" onCopy={(event) => event.preventDefault()} onContextMenu={(event) => event.preventDefault()}>
    <header className="eios-nav">
      ...
      <nav aria-label="EIOS page navigation"><a href="/">Home</a><a href="/services">Services</a><a href="/apps">Applications</a><a href="/academy">Academy</a><a href="/about">About</a><a href="/contact">Contact</a><a href="#connect" className="eios-nav-cta">Engage with EIOS</a></nav>
    </header>

    <section className="eios-hero" id="experience">
      <div className="eios-orbit orbit-one" /><div className="eios-orbit orbit-two" />
      <div className="eios-hero-copy"><p className="eios-eyebrow">PROPERTY OF OBSERRA · ENTERPRISE INTELLIGENCE OPERATING SYSTEM</p><h1>From fragmented context<br />to <em>commercially accountable execution.</em></h1><p className="eios-lede">EIOS is the enterprise intelligence and execution system from OBSERRA EXECUTIVE PROTECTION &amp; INTELLIGENCE LLC. It helps leadership teams connect systems, quantify risk and business impact, authorize action, and verify outcome integrity without replacing core enterprise platforms.</p><div className="eios-actions"><a href="#showcase" className="eios-button">Review EIOS product views</a><a href="#connect" className="eios-text-link">Request enterprise briefing</a></div><p className="eios-restriction">EIOS visuals and product artifacts are proprietary. Public views are controlled representations and do not expose customer data, credentials, or live production architecture.</p></div>
      <div className="eios-hero-art"><ProductFrame view={selected} prominent /></div>
    </section>

    <section className="eios-proof"><p>THE EIOS ADVANTAGE</p><div><span>Connect fragmented context</span><span>Govern authorized action</span><span>Verify outcomes independently</span><span>Measure risk and business value</span></div></section>

    <section className="eios-outcomes" id="outcomes"><div className="section-intro"><p className="eios-eyebrow">THE EIOS DECISION PATH</p><h2>Less dashboard noise.<br />More decisions with measurable commercial impact.</h2></div><div className="outcome-grid">{outcomes.map(([number, title, copy]) => <article key={title}><span>{number}</span><h3>{title}</h3><p>{copy}</p></article>)}</div></section>
    ...
    <section className="eios-showcase" id="showcase"><div className="section-intro"><p className="eios-eyebrow">PRODUCT SHOWCASE</p><h2>Enterprise intelligence<br />that leadership can act on.</h2><p>Review controlled EIOS product views designed to communicate decision flow, accountability, and business impact without exposing customer environments or protected implementation details.</p></div><div className="showcase-layout"><div className="view-selector" role="tablist" aria-label="EIOS product views">{productViews.map((view) => <button key={view.id} role="tab" aria-selected={selected.id === view.id} className={selected.id === view.id ? "active" : ""} onClick={() => setSelected(view)}><small>{view.eyebrow}</small><strong>{view.title}</strong><span>Open product view</span></button>)}</div><ProductFrame view={selected} /></div></section>
    ...
    <section className="eios-connect" id="connect"><p className="eios-eyebrow">ENGAGE WITH EIOS</p><h2>Turn enterprise complexity into accountable commercial momentum.</h2><p>Start with an enterprise EIOS briefing to align decision priorities, control boundaries, integration scope, deployment pathway, and measurable business outcomes with your operating model.</p><div className="eios-engagements"><a href="mailto:info@obserrallc.com?subject=EIOS%20Enterprise%20Scoping%20Session" className="eios-button">Book enterprise scoping session</a><a href="mailto:info@obserrallc.com?subject=EIOS%20Executive%20Briefing" className="eios-outline-button">Request executive briefing</a></div><small>Briefings and scoping sessions are commercial planning engagements for enterprise adoption and do not expose protected implementation assets.</small></section>
