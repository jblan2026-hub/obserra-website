...
export default function EiosShowcase() {
  ...
  return <main className="eios-page" onCopy={(event) => event.preventDefault()} onContextMenu={(event) => event.preventDefault()}>
    ...
    <section className="eios-hero" id="experience">
      ...
      <div className="eios-hero-copy"><p className="eios-eyebrow">PROPERTY OF OBSERRA · ENTERPRISE INTELLIGENCE OPERATING SYSTEM</p><h1>Governed intelligence.<br /><em>Verified enterprise action.</em></h1><p className="eios-lede">EIOS is an enterprise ready operating system that unifies intelligence, risk, governance, and accountable action across the decisions that matter most.</p><div className="eios-actions"><a href="#capabilities" className="eios-button">Explore EIOS capabilities <span>→</span></a><a href="#connect" className="eios-text-link">Engage with Obserra <span>→</span></a></div><p className="eios-restriction">EIOS visual and product materials are proprietary to Obserra. Public views are controlled marketing representations. No customer data, credentials, or live production connections are displayed.</p></div>
    ...
    <section className="eios-connect" id="connect"><p className="eios-eyebrow">ENGAGE WITH EIOS</p><h2>Bring governed intelligence to your enterprise.</h2><p>Choose the engagement path that fits your organization: an executive Partner Session to align priorities, or an EIOS Pilot Program to validate the experience against a focused business challenge.</p><div className="eios-engagements"><a href="mailto:info@obserrallc.com?subject=EIOS%20Partner%20Session" className="eios-button">Schedule a Partner Session <span>→</span></a><a href="mailto:info@obserrallc.com?subject=EIOS%20Pilot%20Program" className="eios-outline-button">Explore the Pilot Program <span>→</span></a></div><small>Partner Sessions and Pilot Programs are commercial engagement options for enterprise adoption. They do not limit the EIOS platform capability scope.</small></section>

    <footer className="eios-footer"><Image src="/brand/obserra-logo.png" width={176} height={34} alt="Obserra Executive Protection and Intelligence LLC" /><p>© Obserra Executive Protection &amp; Intelligence LLC. EIOS and related visual and product materials are proprietary to Obserra. Unauthorized reproduction, distribution, or use is prohibited.</p></footer>
  </main>;
}
