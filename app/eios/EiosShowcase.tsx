...
export default function EiosShowcase() {
  ...
  return <main className="eios-page" onCopy={(event) => event.preventDefault()} onContextMenu={(event) => event.preventDefault()}>
    <header className="eios-nav">
      <a className="eios-brand" href="/"><Image src="/brand/obserra-logo.png" width={218} height={42} alt="Obserra Executive Protection and Intelligence LLC" /><span><b>EIOS</b></span></a>
    ...
    <section className="eios-hero" id="experience">
      ...
      <div className="eios-hero-copy"><p className="eios-eyebrow">PROPERTY OF OBSERRA Â· ENTERPRISE INTELLIGENCE OPERATING SYSTEM</p><h1>Governed intelligence.<br /><em>Verified enterprise action.</em></h1><p className="eios-lede">EIOS is an enterprise ready operating system that unifies intelligence, risk, governance, and accountable action across the decisions that matter most.</p><div className="eios-actions"><a href="#capabilities" className="eios-button">Explore EIOS capabilities <span>â†’</span></a><a href="#connect" className="eios-text-link">Engage with Obserra <span>â†’</span></a></div><p className="eios-restriction">EIOS visual and product materials are proprietary to Obserra. Public views are controlled marketing representations. No customer data, credentials, or live production connections are displayed.</p></div>
    ...
    <section className="eios-capabilities" id="capabilities"><div className="section-intro"><p className="eios-eyebrow">FULL ENTERPRISE CAPABILITY SCOPE</p><h2>One operating system.<br />Every critical decision domain.</h2><p>EIOS is designed as a connected enterprise intelligence platform, not a collection of isolated dashboards. Capability engagement and configuration align to each organizationâ€™s operating model and priorities.</p></div><div className="capability-grid">{capabilities.map(([title, copy], index) => <article key={title}><span>{String(index + 1).padStart(2, "0")}</span><h3>{title}</h3><p>{copy}</p><em>Explore capability <b>â†’</b></em></article>)}</div></section>

    <section className="eios-showcase" id="showcase"><div className="section-intro"><p className="eios-eyebrow">PRODUCT SHOWCASE</p><h2>Enterprise intelligence<br />that makes its case.</h2><p>Explore controlled EIOS demonstration views. Each communicates what leaders need to know without exposing customer environments or proprietary implementation details.</p></div><div className="showcase-layout"><div className="view-selector" role="tablist" aria-label="EIOS product views">{productViews.map((view) => <button key={view.id} role="tab" aria-selected={selected.id === view.id} className={selected.id === view.id ? "active" : ""} onClick={() => setSelected(view)}><small>{view.eyebrow}</small><strong>{view.title}</strong><span>View experience â†’</span></button>)}</div><ProductFrame view={selected} /></div></section>
