  ...
  return (
    ...
    >
      ...
      <section className="eios-hero" id="experience">
        <div className="eios-hero-copy">
          ...
          <p className="eios-lede">
            EIOS gives leadership one governed operating picture across cybersecurity, protective intelligence, enterprise risk, and accountable execution. It connects existing systems, clarifies business impact, controls authorized action, and preserves the evidence needed to verify results.
          </p>
          ...
          <p className="eios-restriction">
            Public EIOS views are controlled product representations. They do not expose customer data, credentials, live production architecture, or protected implementation details.
          </p>
        </div>
      ...
      <section className="eios-proof">
        ...
        <div>
          <span>Connect enterprise context</span>
          <span>Prioritize material decisions</span>
          <span>Govern authorized action</span>
          <span>Verify measurable outcomes</span>
        </div>
      </section>
      ...
      <section className="eios-outcomes" id="outcomes">
        <div className="section-intro">
          ...
          <h2>From fragmented signals to accountable execution.</h2>
        </div>
      ...
      <section className="eios-capabilities" id="capabilities">
        <div className="section-intro">
          <p className="eios-eyebrow">ENTERPRISE CAPABILITY SCOPE</p>
          <h2>One governed intelligence layer across critical decision domains.</h2>
          <p>EIOS supports the same enterprise domains {LEGAL_ENTITY_NAME} addresses through advisory and operations: cybersecurity, protective intelligence, identity, governance, resilience, business impact, and executive accountability.</p>
        </div>
        <div className="capability-grid">
          {capabilities.map(([title, copy], index) => (
            <button key={title} type="button" className={selectedCapabilityIndex === index ? "capability-card active" : "capability-card"} onClick={() => exploreCapability(index)} aria-expanded={selectedCapabilityIndex === index} aria-controls="capability-detail">
              <span>{String(index + 1).padStart(2, "0")}</span><h3>{title}</h3><p>{copy}</p><em>Explore capability <b>→</b></em>
            </button>
          ))}
        </div>
        {selectedCapability && (
          <div className="capability-detail" id="capability-detail" aria-live="polite">
            <div><p className="eios-eyebrow">EIOS CAPABILITY {String(selectedCapabilityIndex! + 1).padStart(2, "0")}</p><h3>{selectedCapability[0]}</h3><p>{selectedCapability[1]}</p></div>
            <div className="capability-detail-actions">
              <button type="button" className="eios-button" onClick={() => document.getElementById("showcase")?.scrollIntoView({ behavior: "smooth", block: "start" })}>Open related product view</button>
              <a className="eios-outline-button" href={`mailto:info@obserrallc.com?subject=${encodeURIComponent(`EIOS ${selectedCapability[0]} inquiry`)}`}>Discuss this capability</a>
            </div>
          </div>
        )}
      </section>
      ...
      <section className="eios-showcase" id="showcase">
        <div className="section-intro">
          ...
          <h2>Decision intelligence leadership can act on.</h2>
          <p>Review controlled EIOS product views that communicate decision flow, accountability, and business impact without exposing customer environments or protected implementation details.</p>
        </div>
        <div className="showcase-layout">
          <div className="view-selector" role="tablist" aria-label="EIOS product views">
            {productViews.map((view) => (
              <button key={view.id} role="tab" aria-selected={selected.id === view.id} className={selected.id === view.id ? "active" : ""} onClick={() => setSelected(view)}>
                <small>{view.eyebrow}</small><strong>{view.title}</strong><span>Open product view</span>
              </button>
            ))}
          </div>
      ...
      <section className="eios-boundary">
        <div><p className="eios-eyebrow">ENTERPRISE READY. DISCIPLINED BY DESIGN.</p><h2>Show the product value without exposing the proprietary blueprint.</h2></div>
        <p>The public EIOS experience communicates capability, outcomes, and design principles. It does not expose platform code, architecture, customer data, live connectors, operational credentials, or internal workflows.</p>
      </section>

      <section className="eios-connect" id="connect">
        <p className="eios-eyebrow">ENGAGE WITH EIOS</p>
        <h2>Bring fragmented risk and operating context into one accountable decision process.</h2>
        <p>Start with an enterprise briefing to align leadership priorities, integration scope, decision authority, deployment boundaries, and measurable business outcomes.</p>
        <div className="eios-engagements">
          <a href="mailto:info@obserrallc.com?subject=EIOS%20Enterprise%20Scoping%20Session" className="eios-button">Book enterprise scoping session</a>
          <a href="mailto:info@obserrallc.com?subject=EIOS%20Executive%20Briefing" className="eios-outline-button">Request executive briefing</a>
        </div>
        <small>Briefings and scoping sessions are commercial planning engagements for enterprise adoption and do not expose protected implementation assets.</small>
      </section>

      <footer className="eios-footer">
        <Image src="/brand/obserra-logo.png" width={176} height={34} alt={LEGAL_ENTITY_NAME} />
        <p>Copyright {LEGAL_ENTITY_NAME}. EIOS and related visual and product materials are proprietary to {LEGAL_ENTITY_NAME}. Unauthorized reproduction, distribution, or use is prohibited.</p>
      </footer>
    </main>
  );
}
...
function ProductFrame({ view, prominent = false }: { view: typeof productViews[number]; prominent?: boolean }) {
  return (
    <figure className={`product-frame ${prominent ? "prominent" : ""}`}>
      <div className="frame-top"><span>OBSERRA EIOS</span><span>CONTROLLED PRODUCT VIEW</span></div>
      <div className="product-image">
        <Image src={view.image} alt={view.alt} fill sizes={prominent ? "(max-width: 900px) 90vw, 54vw" : "(max-width: 900px) 90vw, 63vw"} priority={view.id === "overview"} />
        <div className="property-watermark">PROPERTY OF {LEGAL_ENTITY_NAME}</div>
      </div>
      <figcaption><p>{view.eyebrow}</p><strong>{view.title}</strong><span>{view.copy}</span></figcaption>
    </figure>
  );
}
