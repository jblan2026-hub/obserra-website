...
export default async function AdminPage() {
  ...
  return <main className="admin-shell">
    <header><img src="/brand/obserra-logo.png" alt="Obserra Executive Protection and Intelligence LLC" /><span>SECURED OWNER CONTROL ROOM</span></header>
    <section className="admin-hero"><p>Signed in owner: {emails[0]}</p><h1>Obserra site administration</h1><p>Protected Academy controls, commerce performance, and release readiness. Public website content remains managed through the private Obserra source repository and Vercel project access.</p></section>
    ...
    <section className="admin-section"><h2>Website activity</h2><p>Privacy-first page views, top pages, traffic sources, and the Academy checkout-started click event are collected by Vercel Web Analytics. The metrics remain in the private Vercel account rather than being exposed through the public site.</p><p><a href="https://vercel.com/obserra/obserra-integrated-services/analytics" target="_blank" rel="noreferrer">Open private Web Analytics</a></p></section>
    <section className="admin-section"><h2>Course release control</h2><div className="admin-course-list">{courses.map((course) => <article key={course.id}><div><span>{course.department}</span><h3>{course.title}</h3><p>{course.modules.length} interactive lessons, 25 question final, and 80 percent certificate threshold</p></div><div><b>{metrics.coursesByEnrollment[course.id] ?? 0} historic enrollments</b><a href={`/academy/${course.id}`}>Open public course page</a></div></article>)}</div></section>
    <section className="admin-section"><h2>Owner access path</h2><p>Only the approved owner email can open this administration area. The allowlist is stored as a protected Vercel environment variable and is never exposed in public website code. Purchasers receive a signed, server-verified paid course session after checkout; it cannot grant administration access through checkout, invitation, or a public route.</p></section>
  </main>;
}
