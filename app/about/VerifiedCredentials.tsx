...
function EcCouncilArtwork({ name, image }: { name: string; image: string }) {
  const [failed, setFailed] = useState(false);
  return (
    <div className="ec-council-artwork-frame">
      {!failed ? <img src={image} alt={`${name} official EC-Council badge`} loading="lazy" draggable={false} onError={() => setFailed(true)} /> : null}
      {failed ? <div className="ec-council-fallback-mark" role="img" aria-label={`${name} EC-Council credential`}><span>EC</span><strong>{name}</strong><small>COUNCIL VERIFIED</small></div> : null}
...
export default function VerifiedCredentials() {
  ...
  return (
    <section className="verified-credentials-gallery" aria-labelledby="verified-credentials-title">
      <div className="verified-credentials-heading"><p>VERIFIED EXECUTIVE CREDENTIALS</p><h2 id="verified-credentials-title">Issuer-backed qualifications in cybersecurity, risk, privacy, audit, AI governance, and protective services.</h2><span>Use each issuer link to verify the credential or professional license at its official source.</span></div>
      <h3 className="verified-credentials-group-title">Credly verified credentials</h3>
      <div className="verified-credentials-grid">{credly.map(([name, fullName, issuer, badgeId]) => <article className="verified-credential-card" key={badgeId}><div className="verified-credential-embed" data-iframe-width="150" data-iframe-height="270" data-share-badge-id={badgeId} data-share-badge-host="https://www.credly.com" /><div className="verified-credential-detail"><span>{issuer}</span><strong>{name}</strong><p>{fullName}</p></div></article>)}</div>
      <h3 className="verified-credentials-group-title">EC-Council verified credentials</h3>
      <div className="verified-credentials-grid ec-council-credentials-grid">{ec.map(([name, fullName, image, verify]) => <article className="verified-credential-card ec-council-credential-card" key={name}><a className="ec-council-badge-artwork protected-credential-artwork" href={verify} target="_blank" rel="noreferrer" aria-label={`Verify ${name}`} onContextMenu={(event) => event.preventDefault()} onDragStart={(event) => event.preventDefault()}><EcCouncilArtwork name={name} image={image} /></a><div className="verified-credential-detail"><span>EC-Council</span><strong>{name}</strong><p>{fullName}</p><a className="verified-credential-verify-link" href={verify} target="_blank" rel="noreferrer">Verify credential</a></div></article>)}</div>
      <h3 className="verified-credentials-group-title">Florida FDACS professional licenses</h3>
      <div className="verified-credentials-grid florida-license-grid">{licenses.map(([type, number, licensedName]) => <article className="verified-credential-card florida-license-card" key={number}><div className="fdacs-license-mark" aria-label="Florida Department of Agriculture and Consumer Services"><span>FLORIDA</span><strong>FDACS</strong><small>LICENSED</small></div><div className="verified-credential-detail"><span>Florida FDACS</span><strong>{type}</strong><p>{number}</p><small className="florida-license-name">{licensedName}</small><a className="verified-credential-verify-link" href="https://licensing.fdacs.gov/access/individual.aspx" target="_blank" rel="noreferrer">Verify with Florida FDACS</a></div></article>)}</div>
