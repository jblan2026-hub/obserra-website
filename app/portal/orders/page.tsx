  ...
  return (
    <header className={nav}>
      ...
      <nav className="grid w-full grid-cols-2 gap-2 md:flex md:w-auto md:flex-wrap md:items-center md:gap-4">
        ...
        {billing ? <a className={goldButton} href="/api/apps/billing-portal">Manage billing</a> : null}
      </nav>
    </header>
  );
}
  ...
  return (
    <main className={shell}>
      ...
      <section className="bg-[radial-gradient(circle_at_88%_8%,#1e648b4a,transparent_34%)] px-[max(5vw,24px)] pb-9 pt-16 md:pt-20">
        ...
        <span className="mt-5 block max-w-4xl text-lg leading-8 text-[#b8d5e6]">Records are loaded directly from Stripe using your authenticated Obserra account. No order, invoice, subscription, balance, or receipt is generated by the website.</span>
      </section>
      ...
      <section className="px-[max(5vw,24px)] py-8">
        ...
        {subscriptions.length ? <div className="mt-5 grid grid-cols-[repeat(auto-fit,minmax(290px,1fr))] gap-4">{subscriptions.map((subscription) => {
          ...
          return <article key={subscription.id} className="rounded-2xl border border-[#72bada33] bg-gradient-to-br from-[#0d3556] to-[#071e33] p-5"><header className="grid gap-3 sm:flex sm:items-start sm:justify-between"><small className={eyebrow}>{subscription.status.toUpperCase()}</small><strong className="text-xl sm:text-right">{product?.name || appSlug || "Obserra subscription"}</strong></header><dl className="my-5 grid gap-3">{[["Plan", subscription.metadata.plan || item?.price.recurring?.interval || "Not available"],["Deployment", subscription.metadata.deploymentModel || "Not specified"],["Started", dateFromUnix(subscription.start_date)],["Current period", dateFromUnix(item?.current_period_end)]].map(([term, detail]) => <div key={term} className="flex justify-between gap-4 border-b border-[#72bada22] pb-2"><dt className="text-[#9ccbe2]">{term}</dt><dd className="m-0 text-right">{detail}</dd></div>)}</dl><footer className="flex flex-wrap gap-3">{appSlug ? <Link className={goldButton} href="/portal/applications">Open fulfillment</Link> : <Link className={goldButton} href="/store">View offering</Link>}<a className={goldButton} href="/api/apps/billing-portal">Manage subscription</a></footer></article>;
