...
function canonicalRedirect(request: NextRequest) {
  ...
  const shouldRedirectToCanonical = host === "obserrallc.com" || host === PROJECT_VERCEL_HOST || (process.env.VERCEL_ENV === "production" && host.endsWith(".vercel.app"));
...
function configurationGate(request: NextRequest) {
  ...
  const url = new URL(request.url);
  const requiresAuthentication = isProtected(request) || url.pathname.startsWith("/sign-in") || url.pathname.startsWith("/sign-up");
