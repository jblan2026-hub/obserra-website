...
function config() {
  ...
  if (!floridaClassDSupabaseServerConfigAuthorized(url, key)) throw new FloridaClassDExamError("Exam monitoring service is not configured.", 503, "FDACS_EXAM_MONITORING_NOT_CONFIGURED");
  return { key, url };
}
