...
async function rpc<T>(name:string,body:Record<string,unknown>){await ensureApplicationsRuntimeSecrets();const c=config();const r=await fetch(`${c.url}/rest/v1/rpc/${name}`,{method:"POST",cache:"no-store",redirect:"error",headers:{apikey:c.key,...(c.jwt?{authorization:`Bearer ${c.key}`}:{ }),"content-type":"application/json",accept:"application/json"},body:JSON.stringify(body),signal:AbortSignal.timeout(10_000)});if(!r.ok)throw new AiMarketplaceCommerceError("Marketplace durable commerce is unavailable.",r.status);try{return await r.json() as T;}catch{throw new AiMarketplaceCommerceError("Marketplace durable commerce returned invalid data.");}}
...
export async function aiMarketplaceCustomer(subject:string,tenant:string){identity(subject,tenant);const v=await rpc<{stripeCustomerId:string}|null>("obserra_ai_marketplace_customer",{p_subject_id:subject,p_tenant_id:tenant});if(v&&!CUSTOMER.test(v.stripeCustomerId))throw new AiMarketplaceCommerceError("Marketplace customer record is invalid.");return v;}
export async function bindAiMarketplaceCustomer(subject:string,tenant:string,customer:string){identity(subject,tenant);if(!CUSTOMER.test(customer))throw new AiMarketplaceCommerceError("Marketplace customer is invalid.",400);return rpc<{stripeCustomerId:string}>("obserra_ai_marketplace_bind_customer",{p_subject_id:subject,p_tenant_id:tenant,p_stripe_customer_id:customer});}
...
export async function marketplaceV12CustomerInventory(subject:string,tenant:string){
  ...
  const value=await rpc<unknown>("obserra_ai_marketplace_v12_entitlements",{p_subject_id:subject,p_tenant_id:tenant});
...
export async function marketplaceV12BridgeEnrollment(bridgeId: string) {
  ...
  const value = await rpc<{ bridgeId: string; publicKeyPem: string; platform: string } | null>("obserra_ai_marketplace_v12_bridge_enrollment", { p_bridge_id: bridgeId });
...
export async function lookupMarketplaceV12InstallGrant(input: { grantId: string; bridgeId: string }) {
  ...
  return rpc<{ productId: string; catalogRevision: string; artifactSha256: string; platform: string; installProfile: string; correlationId: string } | null>("obserra_ai_marketplace_lookup_v12_install_grant", { p_grant_id: input.grantId, p_bridge_id: input.bridgeId });
}
...
export async function consumeMarketplaceV12InstallGrant(input: { grantId: string; bridgeId: string; receiptCorrelationId: string }) {
  ...
  return rpc("obserra_ai_marketplace_consume_v12_install_grant", { p_grant_id: input.grantId, p_bridge_id: input.bridgeId, p_receipt_correlation_id: input.receiptCorrelationId });
}
