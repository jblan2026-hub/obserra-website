...
type ChallengeRow = { id: string; playback_session_id: string; challenge_code_digest: string; status: string; expires_at: string; attempt_count: number };
...
export function floridaClassDRecordedMakeupEnabled() {
  return floridaClassDMakeupEnabled() && enabled(process.env.OBSERRA_FDACS_CLASS_D_RECORDED_MAKEUP_ENABLED) && Boolean(process.env.OBSERRA_FDACS_RECORDED_MEDIA_ORIGIN?.trim());
}
...
function config() {
  ...
  if (!floridaClassDSupabaseServerConfigAuthorized(url, key) || !mediaOrigin.startsWith("https://")) {
...
async function request<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  const response = await fetch(`${url}/rest/v1/${path}`, {
    ...
    cache: "no-store",
    redirect: "error",
    headers: { accept: "application/json", apikey: key, authorization: `Bearer ${key}`, "content-type": "application/json", ...(init.headers ?? {}) },
  ...
  if (raw) {
    ...
    catch { throw new FloridaClassDMakeupError("Recorded make-up persistence returned an invalid response.", 502, "FDACS_RECORDED_MAKEUP_INVALID_RESPONSE"); }
  }
  if (!response.ok) {
...
async function assignmentForStudent(userId: string, assignmentId: string) {
  ...
  const rows = await request<AssignmentRow[]>(`fdacs_class_d_makeup_assignments?${new URLSearchParams({ select: "id,enrollment_id,delivery_method,assigned_minutes,recording_asset_reference,status", id: `eq.${assignmentId}`, limit: "1" })}`);
  const assignment = rows[0];
  if (!assignment || assignment.delivery_method !== "recorded_makeup" || !assignment.recording_asset_reference) throw new FloridaClassDMakeupError("Recorded make-up assignment is not available.", 404, "FDACS_RECORDED_MAKEUP_ASSIGNMENT_NOT_AVAILABLE");
  if (["cancelled", "failed", "certified"].includes(assignment.status)) throw new FloridaClassDMakeupError("Recorded make-up assignment is closed.", 409, "FDACS_RECORDED_MAKEUP_ASSIGNMENT_CLOSED");
  ...
  if (!enrollments[0] || enrollments[0].clerk_user_id !== userId) throw new FloridaClassDMakeupError("You are not authorized for this recorded make-up assignment.", 403, "FDACS_RECORDED_MAKEUP_STUDENT_MISMATCH");
  return { ...assignment, recording_asset_reference: assignment.recording_asset_reference };
}

function validatedAssetReference(assetReference: string) {
  if (!/^[A-Za-z0-9][A-Za-z0-9._/-]{2,240}$/.test(assetReference) || assetReference.includes("..")) throw new FloridaClassDMakeupError("Recorded asset reference is invalid.", 500, "FDACS_RECORDED_MAKEUP_ASSET_INVALID");
  return assetReference;
}

export async function resolveFloridaClassDRecordedMedia(userId: string, assignmentId: string, playbackSessionId: string) {
  requireUuid(playbackSessionId, "playback session id");
  const assignment = await assignmentForStudent(userId, assignmentId);
  const sessions = await request<PlaybackRow[]>(`fdacs_class_d_recorded_playback_sessions?${new URLSearchParams({ select: "id,assignment_id,enrollment_id,clerk_user_id,status", id: `eq.${playbackSessionId}`, assignment_id: `eq.${assignment.id}`, clerk_user_id: `eq.${userId}`, status: "in.(active,paused,challenge_required)", limit: "1" })}`);
  if (!sessions[0]) throw new FloridaClassDMakeupError("Recorded playback session is not authorized for media delivery.", 403, "FDACS_RECORDED_MAKEUP_MEDIA_NOT_AUTHORIZED");
  const { mediaOrigin } = config();
  return `${mediaOrigin}/${validatedAssetReference(assignment.recording_asset_reference)}`;
}
...
export async function startFloridaClassDRecordedMakeup(userId: string, clerkSessionId: string | null, input: { assignmentId: string; browserInstanceId: string; correlationId: string }) {
  ...
  const active = await request<PlaybackRow[]>(`fdacs_class_d_recorded_playback_sessions?${new URLSearchParams({ select: "id,assignment_id,enrollment_id,clerk_user_id,clerk_session_id,browser_instance_id,status,playback_position_seconds,verified_watch_seconds,uncredited_seconds,started_at,last_heartbeat_at,challenge_due_at,completed_at", enrollment_id: `eq.${assignment.enrollment_id}`, status: "in.(active,paused,challenge_required)", order: "created_at.desc", limit: "1" })}`);
  if (active[0]) {
    if (active[0].browser_instance_id !== input.browserInstanceId || active[0].clerk_session_id !== clerkSessionId) throw new FloridaClassDMakeupError("Recorded make-up is already active on another device or session.", 409, "FDACS_RECORDED_MAKEUP_SINGLE_DEVICE");
    return { playback: active[0], mediaUrl: `/api/florida-class-d/recorded-makeup/media?assignmentId=${encodeURIComponent(assignment.id)}&playbackSessionId=${encodeURIComponent(active[0].id)}`, requiredWatchSeconds: assignment.assigned_minutes * 60, resumed: true };
  }
  const rows = await request<PlaybackRow[]>("fdacs_class_d_recorded_playback_sessions", { method: "POST", headers: { prefer: "return=representation" }, body: JSON.stringify({ assignment_id: assignment.id, enrollment_id: assignment.enrollment_id, clerk_user_id: userId, clerk_session_id: clerkSessionId, browser_instance_id: input.browserInstanceId, status: "active", correlation_id: input.correlationId }) });
  ...
  return { playback, mediaUrl: `/api/florida-class-d/recorded-makeup/media?assignmentId=${encodeURIComponent(assignment.id)}&playbackSessionId=${encodeURIComponent(playback.id)}`, requiredWatchSeconds: assignment.assigned_minutes * 60, resumed: false };
}
...
export async function heartbeatFloridaClassDRecordedMakeup(userId: string, input: { playbackSessionId: string; browserInstanceId: string; observedPositionSeconds: number; pageVisible: boolean; correlationId: string }) {
  requireUuid(input.playbackSessionId, "playback session id"); requireUuid(input.correlationId, "correlation id");
  ...
  return request<Record<string, unknown>>("rpc/fdacs_class_d_record_recorded_playback_heartbeat", { method: "POST", body: JSON.stringify({ p_playback_session_id: input.playbackSessionId, p_clerk_user_id: userId, p_browser_instance_id: input.browserInstanceId, p_observed_position_seconds: input.observedPositionSeconds, p_page_visible: input.pageVisible, p_correlation_id: input.correlationId }) });
}
...
export async function issueFloridaClassDRecordedMakeupChallenge(userId: string, input: { playbackSessionId: string; correlationId: string }) {
  requireUuid(input.playbackSessionId, "playback session id"); requireUuid(input.correlationId, "correlation id");
  const sessions = await request<PlaybackRow[]>(`fdacs_class_d_recorded_playback_sessions?${new URLSearchParams({ select: "id,assignment_id,enrollment_id,clerk_user_id,status", id: `eq.${input.playbackSessionId}`, clerk_user_id: `eq.${userId}`, limit: "1" })}`);
  ...
  const rows = await request<ChallengeRow[]>("fdacs_class_d_recorded_playback_challenges", { method: "POST", headers: { prefer: "return=representation" }, body: JSON.stringify({ playback_session_id: session.id, assignment_id: session.assignment_id, enrollment_id: session.enrollment_id, challenge_code_digest: digest(code), expires_at: expiresAt, correlation_id: input.correlationId }) });
...
export async function answerFloridaClassDRecordedMakeupChallenge(userId: string, input: { playbackSessionId: string; challengeId: string; answer: string; correlationId: string }) {
  requireUuid(input.playbackSessionId, "playback session id"); requireUuid(input.challengeId, "challenge id"); requireUuid(input.correlationId, "correlation id");
  ...
  const rows = await request<ChallengeRow[]>(`fdacs_class_d_recorded_playback_challenges?${new URLSearchParams({ select: "id,playback_session_id,challenge_code_digest,status,expires_at,attempt_count", id: `eq.${input.challengeId}`, playback_session_id: `eq.${input.playbackSessionId}`, limit: "1" })}`);
  ...
  const expired = Date.parse(challenge.expires_at) <= Date.now(); const correct = !expired && digest(input.answer.trim()) === challenge.challenge_code_digest; const attempts = challenge.attempt_count + 1; const passed = correct; const failed = expired || (!correct && attempts >= 2); const status = passed ? "passed" : failed ? (expired ? "expired" : "failed") : "pending";
...
export async function completeFloridaClassDRecordedMakeup(userId: string, input: { playbackSessionId: string; correlationId: string }) {
  ...
  const session = sessions[0];
  if (!session) throw new FloridaClassDMakeupError("Recorded make-up session was not found.", 404, "FDACS_RECORDED_MAKEUP_SESSION_NOT_FOUND");
  const assignment = await assignmentForStudent(userId, session.assignment_id); const requiredSeconds = assignment.assigned_minutes * 60;
  ...
  const now = new Date().toISOString(); const evidenceReference = `recorded-playback:${session.id}`;
