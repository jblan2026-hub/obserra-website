...
12. Gate 37 publication completed through PR #82. All fourteen candidate checks passed on GitHub-verified head `688a5845b013a078c9a8c4971af323157800d8bc`; GitHub merged verified production SHA `2dde838ee176e6f450abeca2daad96ab377ed931`. The exact Vercel deployment `dpl_5wuL2pUUGcpgk6z7HgvZyVrQLbpd` built successfully and reached READY, but Vercel retained it in Staged state because required deployment checks failed.

13. Direct Vercel Deployment Details inspection identified each result without disabling it: Clerk DNS was incomplete for `obserrallc.com`; a second Clerk status did not match a Vercel alias; `TypeCheck` was skipped because `package.json` did not define `typecheck`; the required production operational job failed when the previously promoted canonical revision returned identity status `configuration-required`; and custom-domain aliasing was blocked as a consequence of the failed checks.

14. Vercel project settings showed `www.obserrallc.com` as a valid production domain and `obserrallc.com` as a valid 308 redirect. The domain uses third-party authoritative nameservers. The linked Clerk product already records `obserrallc.com` as its production domain, while the inactive Vercel DNS zone contains Clerk DKIM records. The Clerk dashboard required a local human-verification/authentication boundary; remote authentication was not attempted or bypassed. Clerk DNS therefore remains an explicit external owner action, not a technical pass.

15. Gate 38 adds a real `tsc --noEmit` package script and separates the configured Vercel-required pre-promotion job from a separately named exact-SHA post-promotion job. The post-promotion job waits for the intended project, deployment metadata, and exact Git SHA to own the canonical domain before running the full website, Academy, credential, commerce, identity, Florida Class D liveness/readiness, and redirect checks. Human CMMC dispositions remain pending and Florida Class D/CUI activation remains false.
