...
const checks = [
  ...
  [service.includes("throw new Error(\"SaaS validation rate-limit persistence is not configured\")"), "fails closed when persistence is absent"],
  ...
  [route.includes("enforceValidationRateLimit"), "enforces rate limiting before token verification"],
  ...
  [!route.includes("x-obserra-service-key\"),\n      token"), "does not log the validation credential or token"],
  [route.indexOf("enforceValidationRateLimit") < route.indexOf("verifySaasAccessToken(token"), "performs abuse control before cryptographic validation"],
